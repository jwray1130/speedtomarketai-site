...
function topbar(){const r=ROUTES[state.route];const wb=r.surface==='workbench';
 const brand=`<a class="brand" href="#/queue" data-brand="1" title="${state.route==='queue'?'Speed to Market AI home':'Back to the queue'}"><span class="mark">${I.chev}</span><span>Speed to Market <em>AI</em></span></a>`;
 const nav=wb?`<nav class="chapters">${chapterList().map(c=>`<button class="${r.nav===c?'on':''} ${c==='Renewal'&&window.STM_RUNTIME?.freshTab==='Renewal'?'fresh':''}" data-label="${c}">${c}</button>`).join('')}${dealTypeSwitch()}</nav><span class="brk" aria-hidden="true"></span>`:`<nav class="nav">${PLATFORM.map(c=>`<button class="${r.nav===c?'on':''}" data-label="${c}">${c}</button>`).join('')}</nav>`;
...
document.getElementById('chrome').addEventListener('click',e=>{const brandEl=e.target.closest('[data-brand]');if(brandEl){e.preventDefault();e.stopPropagation();/* v9.9.9: decided at click time: home from the queue, back to the queue from anywhere else */if(state.route==='queue'){window.location.href='/';}else{go('queue');}return;}const el=e.target.closest('[data-go],[data-label],[data-act]');if(!el)return;e.preventDefault();
...
/* v9.9.6: every open menu closes on any click anywhere, including inside the page frames */
window.stmCloseMenus=function(){if(state.menu){state.menu=false;renderChrome();}try{window.STM_RUNTIME?.closeActions();}catch(_){}document.querySelectorAll('iframe').forEach(f=>{try{f.contentWindow?.postMessage({stm:1,type:'close-menus'},'*');}catch(_){}});};
document.addEventListener('pointerdown',e=>{if(!e.target.closest('#sysmenu,#sysBtn,#stm-action-menu,.acts'))window.stmCloseMenus();},true);
window.addEventListener('blur',()=>{setTimeout(()=>{if(document.activeElement&&document.activeElement.tagName==='IFRAME')window.stmCloseMenus();},0);});
window.stmHookFrameMenus=function(f){const hook=()=>{try{f.contentDocument?.addEventListener('pointerdown',ev=>{if(!ev.target.closest('[data-menu],.menu,.sysmenu'))window.stmCloseMenus();},true);}catch(_){}};f.addEventListener('load',hook);hook();};
