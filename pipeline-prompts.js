...
- WORKED EXAMPLE for the uniform rule: with 5/1 inceptions, a DOL of 4/30/2026 falls BEFORE the 5/1 month/day and therefore belongs to the 2025-26 band, never the 2026-27 band; a DOL of 5/1/2026 belongs to the 2026-27 band.
- LOB DISCIPLINE (LIABILITY-ONLY AL TABLE): the AL table includes ONLY liability claims: LOB exactly Auto Liability, or any combined designation that contains Auto Liability (e.g., "Auto Liability / APD"). Standalone Auto Physical Damage / APD / comprehensive / collision claims are FIRST-PARTY damage to the insured's own vehicles; they can never penetrate an excess liability layer, so they are EXCLUDED from the AL table, its claim counts, its totals, and its large-loss callouts, and they NEVER appear in the GL table. The GL table includes ONLY General Liability claims. Claims in any other LOB (property, workers compensation, inland marine, cargo, etc.) appear in NEITHER table.
- APD RECONCILIATION: when the run's own commercial-auto total includes first-party APD, state the exclusion in ONE sentence inside the existing Frequency or Severity narrative paragraph (never a new table element): the excluded APD claim count and dollars, so the AL table total plus the stated exclusion equals the run's own commercial-auto total. Apply this LOB assignment before policy-year banding and use it identically in the HTML tables and the loss_history_structured JSON.
...
SECTION SELECTION: ALWAYS include Established Expertise, Loss History, and Attachment Point and Program Structure. ALSO ALWAYS include Commitment to Safety and Operational Controls whenever an A5 Safety Program Review block is present in the input and documents at least one present control; credit ONLY documented present controls, never absences. If A5 is present but documents no creditable controls, omit the section rather than padding. Include any other section ONLY when the input supports it; omit unsupported sections rather than padding. Order sections logically for this account. Each section is a bold Title-Case header followed by bullets.
...
- Commitment to Safety and Operational Controls: mandatory whenever a creditable A5 block is present. Formal written program, dedicated safety leadership, task-specific training, and engineering controls actually named in the A5 Safety Program Review or another source block. When the raw Safety Program (A5) block is present, prefer its specific detail (named programs, dedicated roles and credentials, committee or meeting cadence, named loss-control resources, specific engineering controls) over the compressed summary.
