...
// ============================================================================
// DOCUMENTS VIEW MODULE — Phase 1 Step 1 (visual integration)
// ----------------------------------------------------------------------------
// Renders the Documents stage inside the workbench. Reads files from the
// active submission's snapshot.files. Categorization + color tags persist to
// localStorage for now (per-submission, per-file). Storage bucket integration
// + per-page splitting + persistence to the document_pages table all come in
// later sessions of Phase 1; this is the visual shell.
// ============================================================================

// 14 categories for E&S excess casualty document organization. The id is what
// gets stored; the label is what the UW sees; iconChar is a glyph for visual
// recognition without external icon files.
const DOCS_CATEGORIES = [
  { id: 'all',                name: 'All Documents',     desc: 'View every uploaded file',         iconChar: '⬚' },
  { id: 'correspondence',     name: 'Correspondence',    desc: 'Broker emails and letters',         iconChar: '✉' },
  { id: 'applications',       name: 'Applications',      desc: 'ACORDs and supplementals',          iconChar: '📝' },
  { id: 'loss_history',       name: 'Loss History',      desc: 'Loss runs and claims data',         iconChar: '📉' },
  { id: 'underlying_quotes',  name: 'Underlying Quotes', desc: 'GL · AL · EL · lead · excess',      iconChar: '▣' },
  { id: 'cancellations',      name: 'Cancellations',     desc: 'Cancellation notices',              iconChar: '✕' },
  { id: 'pricing',            name: 'Pricing',           desc: 'Rate calculations and worksheets',  iconChar: '$' },
  { id: 'quotes',             name: 'Quotes',            desc: 'Outgoing Zurich quotations',        iconChar: '▤' },
  { id: 'binders',            name: 'Binders',           desc: 'Policy binders',                    iconChar: '▦' },
  { id: 'policies',           name: 'Policies',          desc: 'Active insurance policies',         iconChar: '⛨' },
  { id: 'endorsements',       name: 'Endorsements',      desc: 'Policy modifications',              iconChar: '✎' },
  { id: 'subjectivities',     name: 'Subjectivities',    desc: 'Pending requirements',              iconChar: '⚠' },
  { id: 'surplus_lines',      name: 'Surplus Lines',     desc: 'Non-admitted policy paperwork',     iconChar: '▥' },
  { id: 'project',            name: 'Project',           desc: 'Project-specific documents',        iconChar: '⌂' },
  { id: 'underwriting',       name: 'Underwriting',      desc: 'Internal UW workpapers',            iconChar: '◈' },
];

const DOCS_COLORS = ['red','maroon','blue','green','yellow','purple','orange','pink','black'];
const DOCS_COLOR_LABELS = {
  red: 'Loss History',
...
// Per-tab state. Lives in memory; the per-file metadata (category/color) is
// persisted to localStorage keyed by submission id + file index/name so it
// survives refresh until the document_pages table lands.
const DOCS_STATE = {
  category: 'all',
  colorFilter: 'all',
  view: 'thumbnail',
  sort: 'newest',
  search: '',
  contextFile: null,  // currently right-clicked file
};

// localStorage key namespace — lets us migrate to the table later without
// stepping on other apps' keys
const DOCS_LS_PREFIX = 'altitude_docs_';

function docsLsKey(submissionId) {
  return DOCS_LS_PREFIX + 'meta_' + submissionId;
}

// Load per-file metadata for a submission from localStorage. Returns a map
// keyed by file index (since legacy snapshot.files entries don't have stable ids).
function docsLoadMeta(submissionId) {
  if (!submissionId) return {};
  try {
    const raw = localStorage.getItem(docsLsKey(submissionId));
    return raw ? JSON.parse(raw) : {};
  } catch (e) {
    console.warn('Could not load docs meta for', submissionId, e);
    return {};
  }
}

function docsSaveMeta(submissionId, meta) {
  if (!submissionId) return;
  try {
    localStorage.setItem(docsLsKey(submissionId), JSON.stringify(meta));
  } catch (e) {
    console.warn('Could not save docs meta for', submissionId, e);
  }
}

// Returns the file list for the active submission, decorated with metadata
// from localStorage. Each file gets a stable docKey based on its name (which
// is what we have to work with on legacy snapshots). When the table lands,
// docKey will become document_pages.id.
function docsGetActiveFiles() {
  const sid = STATE.activeSubmissionId;
  if (!sid) return { files: [], submissionId: null };
  const rec = STATE.submissions.find(s => s.id === sid);
  if (!rec || !rec.snapshot || !Array.isArray(rec.snapshot.files)) {
    return { files: [], submissionId: sid };
  }
  const meta = docsLoadMeta(sid);
  const files = rec.snapshot.files.map((f, idx) => {
    // Stable per-file key: prefer existing id, fall back to name+idx
    const docKey = f.id || (f.name + '__' + idx);
    const m = meta[docKey] || {};
    return {
      ...f,
      docKey,
      _displayName: f.name || ('File ' + (idx + 1)),
      _category: m.category || null,
      _color: m.color || null,
      _tagged: !!m.color,  // any color = tagged
      _idx: idx,
    };
  });
  return { files, submissionId: sid };
...
// Persist a category/color change for a single file
function docsSetMeta(submissionId, docKey, patch) {
  const meta = docsLoadMeta(submissionId);
  meta[docKey] = { ...(meta[docKey] || {}), ...patch };
  // Strip empty/null fields so the blob stays tidy
  if (!meta[docKey].category) delete meta[docKey].category;
  if (!meta[docKey].color) delete meta[docKey].color;
  if (Object.keys(meta[docKey]).length === 0) delete meta[docKey];
  docsSaveMeta(submissionId, meta);
}

// Top-level render entry. Called by showStage('docs').
function renderDocumentsView() {
  const { files, submissionId } = docsGetActiveFiles();
  renderDocsCategoryGrid(files);
  renderDocsGrid(files);
  renderDocsTagsList(files);
  updateDocsHeaderCount(files);
}

function updateDocsHeaderCount(files) {
  const count = files.length;
  const tabBadge = document.getElementById('docsCount');
  if (tabBadge) tabBadge.textContent = count;
  const totalAll = document.getElementById('docsTotalAll');
  if (totalAll) totalAll.textContent = count;
  const totalTagged = document.getElementById('docsTotalTagged');
  if (totalTagged) totalTagged.textContent = files.filter(f => f._tagged).length;
}

function docsFilterFiles(files) {
  let out = files.slice();
  if (DOCS_STATE.category !== 'all') {
    out = out.filter(f => f._category === DOCS_STATE.category);
  }
  if (DOCS_STATE.search) {
    const q = DOCS_STATE.search.toLowerCase();
    out = out.filter(f => (f._displayName || '').toLowerCase().includes(q));
  }
  return docsSortFiles(out);
}

function docsSortFiles(files) {
  const order = DOCS_STATE.sort;
  return files.slice().sort((a, b) => {
    if (order === 'name-asc') return (a._displayName || '').localeCompare(b._displayName || '');
    if (order === 'name-desc') return (b._displayName || '').localeCompare(a._displayName || '');
    if (order === 'type') return (a.type || '').localeCompare(b.type || '');
    if (order === 'oldest') return a._idx - b._idx;
    return b._idx - a._idx;  // newest first (highest idx = most recent in array)
  });
}

function renderDocsCategoryGrid(allFiles) {
  const grid = document.getElementById('docsCatGrid');
  if (!grid) return;
  grid.innerHTML = '';
  DOCS_CATEGORIES.forEach(cat => {
    const count = cat.id === 'all'
      ? allFiles.length
      : allFiles.filter(f => f._category === cat.id).length;
    const card = document.createElement('div');
    card.className = 'docs-cat-card' + (cat.id === DOCS_STATE.category ? ' active' : '');
    card.dataset.cat = cat.id;
    card.innerHTML =
      '<div class="docs-cat-icon">' + cat.iconChar + '</div>' +
      '<div class="docs-cat-body">' +
        '<div class="docs-cat-name">' + escapeHtml(cat.name) + '</div>' +
        '<div class="docs-cat-desc">' + escapeHtml(cat.desc) + '</div>' +
      '</div>' +
      '<div class="docs-cat-count">' + count + '</div>';
    card.onclick = () => docsSelectCategory(cat.id);
    grid.appendChild(card);
  });
}

function docsSelectCategory(catId) {
  DOCS_STATE.category = catId;
  const cat = DOCS_CATEGORIES.find(c => c.id === catId);
  const titleEl = document.getElementById('docsGridTitle');
  if (titleEl) titleEl.textContent = cat ? cat.name : catId;
  renderDocumentsView();
}

function renderDocsGrid(allFiles) {
  const grid = document.getElementById('docsGrid');
  const empty = document.getElementById('docsGridEmpty');
  if (!grid || !empty) return;
  // Clear all children except the empty state
  Array.from(grid.children).forEach(c => { if (c !== empty) c.remove(); });

  const filtered = docsFilterFiles(allFiles);
  const countBadge = document.getElementById('docsGridCount');
  if (countBadge) countBadge.textContent = filtered.length;

  if (filtered.length === 0) {
    empty.style.display = '';
    if (allFiles.length === 0) {
      // No files at all — keep the default "open a submission" empty state
    } else {
      // Files exist but filter excluded them all — adjust message
      const titleEl = empty.querySelector('.docs-grid-empty-title');
      const subEl = empty.querySelector('.docs-grid-empty-sub');
      if (titleEl) titleEl.textContent = 'No documents in this view';
      if (subEl) subEl.textContent = 'Try a different category or clear the search';
    }
    return;
  }
  empty.style.display = 'none';

  filtered.forEach(f => grid.appendChild(buildDocsTile(f)));
}

function buildDocsTile(file) {
  const tile = document.createElement('div');
  tile.className = 'docs-tile';
  if (file._color) {
    tile.classList.add('has-color', 'tag-' + file._color);
  }
  tile.dataset.docKey = file.docKey;

  // Color accent bar on the left
  const colorBar = document.createElement('div');
  colorBar.className = 'docs-tile-color-bar';
  tile.appendChild(colorBar);

  // Thumb area — for Phase 1 Step 1 we show a glyph + filetype badge.
  // PDF thumbnail rendering comes in a later session along with per-page splitting.
  const thumb = document.createElement('div');
  thumb.className = 'docs-tile-thumb';

  const glyph = document.createElement('div');
  glyph.className = 'docs-tile-glyph';
  glyph.textContent = docsTypeGlyph(file);
  thumb.appendChild(glyph);

  const typeBadge = document.createElement('span');
  typeBadge.className = 'docs-tile-type-badge';
  typeBadge.textContent = docsTypeLabel(file);
  thumb.appendChild(typeBadge);

  if (file._category) {
    const catBadge = document.createElement('span');
    catBadge.className = 'docs-tile-cat-badge';
    const cat = DOCS_CATEGORIES.find(c => c.id === file._category);
    catBadge.textContent = cat ? cat.name : file._category;
  ...
  info.className = 'docs-tile-info';
  const name = document.createElement('div');
  name.className = 'docs-tile-name';
  name.textContent = file._displayName;
  name.title = file._displayName;
  info.appendChild(name);

  const meta = document.createElement('div');
  meta.className = 'docs-tile-meta';
  const sizeText = file.size ? docsFormatBytes(file.size) : (file.textDropped ? 'text only' : '—');
  meta.innerHTML =
    '<span class="docs-tile-size">' + escapeHtml(sizeText) + '</span>' +
    (file.classification ? '<span class="docs-tile-class">· ' + escapeHtml(file.classification) + '</span>' : '');
  info.appendChild(meta);
  tile.appendChild(info);

  // Click = preview placeholder; Right-click = context menu for cat/color
  tile.onclick = (e) => {
    e.stopPropagation();
    toast('Preview · "' + file._displayName + '" · full preview ships in next session');
  };
  tile.oncontextmenu = (e) => {
    e.preventDefault();
    DOCS_STATE.contextFile = file;
    showDocsContextMenu(e.clientX, e.clientY);
  };

  return tile;
}

function docsTypeGlyph(file) {
  const t = (file.type || '').toLowerCase();
  const n = (file.name || '').toLowerCase();
  if (t.includes('pdf') || n.endsWith('.pdf')) return '📄';
  if (t.includes('word') || n.endsWith('.docx') || n.endsWith('.doc')) return '📝';
  if (t.includes('sheet') || t.includes('excel') || n.endsWith('.xlsx') || n.endsWith('.xls') || n.endsWith('.csv')) return '📊';
  if (t.includes('image') || /\.(jpe?g|png|tiff?|gif|webp|bmp)$/.test(n)) return '🖼';
  if (t.includes('email') || /\.(eml|msg)$/.test(n)) return '✉';
  return '📁';
}

function docsTypeLabel(file) {
  const n = (file.name || '').toLowerCase();
  const m = n.match(/\.([a-z0-9]{1,5})$/);
  if (m) return m[1].toUpperCase();
  if (file.type) return file.type.split('/').pop().toUpperCase().slice(0, 5);
  return 'FILE';
}
...
function renderDocsTagsList(allFiles) {
  const list = document.getElementById('docsTagsList');
  const empty = document.getElementById('docsTagsEmpty');
  if (!list || !empty) return;
  Array.from(list.children).forEach(c => { if (c !== empty) c.remove(); });

  let tagged = allFiles.filter(f => f._tagged);
  if (DOCS_STATE.colorFilter !== 'all') {
    tagged = tagged.filter(f => f._color === DOCS_STATE.colorFilter);
  }
  document.getElementById('docsTagsCount').textContent = tagged.length;
  ...
  // Group by color so similar tags cluster together
  const colorOrder = [...DOCS_COLORS, null];
  const sorted = [];
  colorOrder.forEach(c => tagged.forEach(f => { if ((f._color || null) === c) sorted.push(f); }));

  sorted.forEach(f => {
    const item = document.createElement('div');
    item.className = 'docs-tagged-item' + (f._color ? ' tag-' + f._color : '');
    const labelText = f._color ? DOCS_COLOR_LABELS[f._color] : '';
    item.innerHTML =
      '<div class="docs-tagged-body">' +
        '<div class="docs-tagged-name">' + escapeHtml(f._displayName) + '</div>' +
        (labelText ? '<span class="docs-tagged-label">' + escapeHtml(labelText) + '</span>' : '') +
      '</div>' +
      '<button class="docs-tagged-remove" title="Remove tag">×</button>';
    item.querySelector('.docs-tagged-remove').onclick = (e) => {
      e.stopPropagation();
      docsSetMeta(STATE.activeSubmissionId, f.docKey, { color: null });
      renderDocumentsView();
    };
...
// Context menu show/hide and action handling
function showDocsContextMenu(x, y) {
  const menu = document.getElementById('docsContextMenu');
  if (!menu) return;
  menu.classList.add('visible');
  // Reposition if it would go off-screen
...
function hideDocsContextMenu() {
  const menu = document.getElementById('docsContextMenu');
  if (menu) menu.classList.remove('visible');
  DOCS_STATE.contextFile = null;
}

function docsContextAction(act) {
  const f = DOCS_STATE.contextFile;
  hideDocsContextMenu();
  if (!f) return;
  const sid = STATE.activeSubmissionId;
  if (!sid) return;
  if (act === 'clear') {
    docsSetMeta(sid, f.docKey, { category: null, color: null });
  } else if (act.startsWith('cat-')) {
    docsSetMeta(sid, f.docKey, { category: act.slice(4) });
  } else if (act.startsWith('color-')) {
    docsSetMeta(sid, f.docKey, { color: act.slice(6) });
  }
  renderDocumentsView();
}

// Color filter dropdown wiring
function docsSelectColorFilter(filter) {
  DOCS_STATE.colorFilter = filter;
  const labelEl = document.getElementById('docsColorFilterLabel');
  if (labelEl) labelEl.textContent = filter === 'all' ? 'All Colors' : DOCS_COLOR_LABELS[filter];
  const dot = document.querySelector('.docs-color-filter-display .docs-color-dot');
  if (dot) dot.className = 'docs-color-dot ' + filter;
  document.querySelectorAll('#docsColorFilterMenu .docs-color-option').forEach(o => {
    o.classList.toggle('active', o.dataset.filter === filter);
  });
  document.getElementById('docsColorFilterMenu')?.classList.remove('visible');
  renderDocumentsView();
}

// One-time event wiring. Called from init below; safe to call multiple times
// because we use addEventListener with named handlers and the listeners are
// attached to elements that only exist once.
let _docsEventsWired = false;
function wireDocsEvents() {
  if (_docsEventsWired) return;
  _docsEventsWired = true;
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      DOCS_STATE.search = v;
      if (searchClear) searchClear.classList.toggle('visible', !!v);
      clearTimeout(searchTimer);
      searchTimer = setTimeout(() => renderDocumentsView(), 100);
    });
  ...
  // Sort menu
  const sortBtn = document.getElementById('docsSortBtn');
  const sortMenu = document.getElementById('docsSortMenu');
  if (sortBtn && sortMenu) {
    sortBtn.onclick = (e) => { e.stopPropagation(); sortMenu.classList.toggle('visible'); };
    document.querySelectorAll('#docsSortMenu .docs-sort-option').forEach(o => {
      o.onclick = () => {
        DOCS_STATE.sort = o.dataset.sort;
  ...
  document.querySelectorAll('#docsViewToggle button').forEach(b => {
    b.onclick = () => {
      DOCS_STATE.view = b.dataset.view;
      document.querySelectorAll('#docsViewToggle button').forEach(x => {
        x.classList.toggle('active', x.dataset.view === b.dataset.view);
      });
      const grid = document.getElementById('docsGrid');
      if (grid) grid.dataset.view = b.dataset.view;
    };
  });

  // Color filter
  const colorBtn = document.getElementById('docsColorFilterBtn');
  const colorMenu = document.getElementById('docsColorFilterMenu');
  if (colorBtn && colorMenu) {
    colorBtn.onclick = (e) => { e.stopPropagation(); colorMenu.classList.toggle('visible'); };
    document.querySelectorAll('#docsColorFilterMenu .docs-color-option').forEach(o => {
      o.onclick = () => docsSelectColorFilter(o.dataset.filter);
    });
  }

  // Context menu
  document.querySelectorAll('#docsContextMenu .docs-ctx-item').forEach(c => {
    c.onclick = () => docsContextAction(c.dataset.act);
  });

  // Global click to close menus
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.docs-sort-wrap')) document.getElementById('docsSortMenu')?.classList.remove('visible');
    if (!e.target.closest('.docs-color-filter')) document.getElementById('docsColorFilterMenu')?.classList.remove('visible');
    if (!e.target.closest('.docs-context-menu, .docs-tile')) hideDocsContextMenu();
  });
}

// Wire events after DOM is ready. Safe to call this multiple times.
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', wireDocsEvents);
} else {
  wireDocsEvents();
}

// Expose for cross-module access (e.g. when a new submission is opened we may
// want to re-render the documents view if it's currently active)
window.renderDocumentsView = renderDocumentsView;

// END DOCUMENTS VIEW MODULE
// ============================================================================
