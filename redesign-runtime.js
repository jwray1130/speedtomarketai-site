...
function setBusy(on,message,epoch=sessionEpoch){if(epoch!==sessionEpoch)return;busy=Math.max(0,busy+(on?1:-1));const el=document.getElementById('loading');el.classList.toggle('on',busy>0);el.setAttribute('aria-hidden',String(busy===0));if(message)el.textContent=message;document.body.dataset.busy=busy?'true':'false';}
