...
async function handleFiles(fileList) {
  ...
  const isIncremental = STATE.pipelineDone || (!!STATE.pipelineRun && Object.keys(STATE.extractions || {}).length > 0);
  const newFiles = [];
...
async function archiveCurrentSubmission(opts) {
  ...
  const snapshot = {
    ...
    pipelineElapsedSeconds: STATE.pipelineElapsedSeconds ?? null,
    _stmRunComplete: window.__STM_SUBMISSION?.completionForSave?.() ?? !!STATE.pipelineDone,
...
function auditExportRowKey8751(row) {
  // v8.7.157: dedupe across merge sources. The same event can appear in
  // STATE.audit, the active submission snapshot, and the local browser ring;
  // including source in the key made cross-source duplicates survive export.
  return [row.submission_id || '', row.pipeline_run || '', row.ts || row.created_at || row.time || '', row.actor || row.category || '', row.action || row.message || '', JSON.stringify(row.meta == null ? '' : row.meta)].join('|');
}
...
function cleanVisibleExtractionText99(mid, text) {
  ...
  if (mid === 'guidelines' && typeof window.cleanGuidelinesVisibleText95 === 'function') {
    t = window.cleanGuidelinesVisibleText95(t);
  }
  if (mid === 'strengths' && typeof window.cleanStrengthsVisibleText95 === 'function') {
    t = window.cleanStrengthsVisibleText95(t);
  }
  return t.trim();
}
